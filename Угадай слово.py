...
def get_word(spisok):
    result = random.choice(spisok).upper()
    return result
...
def play(word):
    ...
    slow('Игра начинается!')
    print()
    time.sleep(0.8)
    print(display_hangman(tries))
    print()
    print()
    slow(f'Длина загаданного слова  -  {len(word)} символов')
    
    while tries != 0:

        while True:
            slow('Введите букву слова ')
            s = input().upper().lstrip()           
            if s not in c:
                c.append(s)
            else:
                slow('Вы уже называли эту букву!')
                continue
            print()           
            if s not in 'АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯ':
                slow('Нужна буква!  (кириллица) ')
            elif s in word_completion:
                slow('Будьте внимательнее, Вы уже называли эту букву!')
            else:
                break
            
        if s in word:
            slow('Поздравляю, Вы угадали букву!')
            new_completion = ''
            ...
            if '*' not in word_completion:
                slow(f'ПОЗДРАВЛЯЮ! ВЫ УГАДАЛИ СЛОВО:  "{word_completion}"')
                time.sleep(0.8)
                print(r'''
                \|||/
                ($ $)
       ...
       '---------------ooO---'
              ...
              (__/ \__)  
                ...
                slow(f'Сейчас ваше слово выглядит вот так - {word_completion}')
                slow('Ваше число попыток остается преждним')
        elif s not in word:
            streak = streak + 1
            tries = tries - 1
            slow('Увы, Вы не угадали букву')
            if streak == need:
                streak = 0
                need += 1
                slow('Ну же, Вы сможете!')
                slow('Я даю вам одну дополнительную попытку [*ПОПЫТКА ДОБАВЛЕНА*]')
                tries += 1
        if tries != 0:
            time.sleep(1)
            print(display_hangman(tries))
            ...
            slow(f'У вас закончились попытки, загаданное слово - {word}')
            time.sleep(0.8)
...
try:    
    play(what_word)
    while True:    
        slow('Желаете сыграть еще раз? ( ДА / НЕТ ) ')
        repeat = input().upper().lstrip()
        if repeat == 'ДА':    
            play(what_word)
        elif repeat == 'НЕТ':
            slow('Спасибо за игру!')
            break
        else:
            slow('Пожалуйста, напишите "да" / "нет" ')
