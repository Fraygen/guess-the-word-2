...
def play(word):
    ...
    while tries != 0:
        while True:
            s = input('Введите букву слова').upper()
            if s not in 'АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯ':
                print('Нужна буква!')
            elif s in word_completion:
                print('Будьте внимательнее, Вы уже называли эту букву!')
            else:
                break
        if s in word:
            ...
            for i in range(len(word_completion)):
                if word[i] == s:
                    word_completion = word_completion.replace('_',s,1)
            print('Сейчас ваше слово выглядит вот так -', word_completion)
            print('Ваше число попыток остается преждним -', tries)
        elif s not in word:
            tries = tries - 1
        ...
        if tries == 0:
            print('У вас закончились попытки, загаданное слово -', word)
            break
